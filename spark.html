...
function insertEmoji(em) {
  ...
  if (_emojiTargetId === "promptTa" && typeof onPromptInput === "function")
    onPromptInput(ta);
  closeEmojiPicker();
}
...
function insertQuoteFormat(targetId) {
  ...
  if (targetId === "promptTa" && typeof onPromptInput === "function")
    onPromptInput(ta);
}
...
function attachMedia(input, type) {
  ...
  if (type === "image") {
    const existing = strip.querySelectorAll(
    ...
    if (files.length > slots && typeof window.showToast === "function")
      window.showToast(
        `Only ${slots} image slot${slots === 1 ? "" : "s"} remaining — added ${allowed.length}`,
      );
...
function attachPromptMedia(input) {
  ...
  if (files.length > slots && typeof window.showToast === "function")
    window.showToast(
      `Only ${slots} slot${slots === 1 ? "" : "s"} left — added ${allowed.length}`,
    );
  allowed.forEach((file) => {
    const reader = new FileReader();
    reader.onload = (e) => {
...
function markAllRead() {
  if (typeof window._markAllReadFn === "function") window._markAllReadFn();
}
...
function closeSearchBar() {
  const wrap = document.getElementById("searchBarWrap");
  const input = document.getElementById("searchInput");
  const userResults = document.getElementById("searchUserResults");
  if (wrap) wrap.classList.remove("open");
  document.body.classList.remove("search-open");
  if (input) input.value = "";
  if (userResults) { userResults.style.display = "none"; userResults.innerHTML = ""; }
...
function onSearchInput(inputEl) {
  ...
  // Always filter the feed
  filterFeedByQuery(q);

  // Debounce user search — only fire if query starts with @ or is 2+ chars
  clearTimeout(_userSearchTimer);
  const userResultsEl = document.getElementById("searchUserResults");

  if (!q) {
    if (userResultsEl) userResultsEl.style.display = "none";
    return;
  }
  ...
  if (userResultsEl) {
    ...
    userResultsEl.innerHTML = '<div class="search-user-loading">Searching users…</div>';
  }
...
async function searchUsers(rawQuery) {
  ...
  try {
    ...
    if (!db || !fns) {
      userResultsEl.innerHTML = '<div class="search-user-empty">Search unavailable.</div>';
      return;
    }

    // Fetch all users and filter client-side (matches fullName, nickname, email prefix)
    ...
    snap.forEach((docSnap) => {
      ...
      const email = (d.emailPrimary || "").toLowerCase();
      if (
        fullName.includes(q) ||
        nickname.includes(q) ||
    ...
    if (matches.length === 0) {
      userResultsEl.innerHTML = '<div class="search-user-empty">No users found for "' + rawQuery + '"</div>';
      return;
    }

    // Render results (max 8)
    const followingSet = window._myFollowingSet || window._followingSet || new Set();
    const html = matches.slice(0, 8).map(({ uid, d }) => {
      const name = d.fullName || d.nickname || "T1ERA User";
      const handle = d.nickname ? "@" + d.nickname : "";
      const photoURL = d.accountStatus?.profilePicture?.url || d.profilePicture?.url || "";
      const initial = name[0].toUpperCase();
      const isFollowing = followingSet.has(uid);

      const avatarHtml = photoURL
        ? `<img src="${photoURL}" alt="" referrerpolicy="no-referrer" onerror="this.style.display='none'">`
        : initial;

      const followLabel = isFollowing ? "Following" : "Follow";
      const followClass = isFollowing ? "search-user-follow-btn following" : "search-user-follow-btn";

      return `<div class="search-user-item" data-uid="${uid}">
        <div class="search-user-av" onclick="event.stopPropagation();window.openPeekCard('${uid}')">${avatarHtml}</div>
        <div class="search-user-info" onclick="event.stopPropagation();window.openPeekCard('${uid}')">
          <div class="search-user-name">${_escHtml(name)}</div>
          <div class="search-user-handle">${_escHtml(handle)}</div>
        </div>
        <button class="${followClass}" data-fuid="${uid}"
          onclick="event.stopPropagation();searchFollowToggle(this,'${uid}')"
        >${followLabel}</button>
      </div>`;
    }).join("");
    ...
    userResultsEl.innerHTML = '<div class="search-user-empty">Could not search users.</div>';
  }
}
...
async function searchFollowToggle(btn, uid) {
  if (!window._followFn) return;
  btn.disabled = true;
  const wasFollowing = btn.classList.contains("following");
  btn.textContent = wasFollowing ? "Unfollowing…" : "Following…";
  await window._followFn({ ownerUid: uid, btn, type: "search" });
  // Refresh follow state from updated set
  const followingSet = window._myFollowingSet || window._followingSet || new Set();
  const nowFollowing = followingSet.has(uid);
  btn.textContent = nowFollowing ? "Following" : "Follow";
  btn.className = nowFollowing ? "search-user-follow-btn following" : "search-user-follow-btn";
  btn.disabled = false;
}
...
function filterFeedByQuery(query) {
  ...
  cards.forEach((card) => {
    ...
    const matches =
      cardText.includes(q) || cardName.includes(q) || cardHandle.includes(q);
